...
int main(int argc, char *argv[])
{
    ...
    std::array<CommandArg, 5> args = {
        CommandArg{"-w", "--width", "Sets the window width", true, [&](const char *w)
                   { windowSize.x = std::stoi(w); }},
        CommandArg{"-h", "--height", "Sets the window height", true, [&](const char *h)
                   { windowSize.y = std::stoi(h); }},
        CommandArg{"-x", "--xpos", "Sets the initial window X position", true, [&](const char *x)
                   { windowPos.x = std::stoi(x); }},
        CommandArg{"-y", "--ypos", "Sets the initial window Y position", true, [&](const char *y)
                   { windowPos.y = std::stoi(y); }},
        CommandArg{"-i", "--input", "Opens Terrapainter with the specified input", true, [&](const char *i)
                   { initialInput = i; }}};
