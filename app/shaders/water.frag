...
void main()
{
	...
	float dx = snoise(vec3(canvasCoords * 50, u_time)) - 0.5;
	float dy = snoise(vec3(canvasCoords * 50, u_time + 1)) - 0.5;
	vec2 d = vec2(dx, dy) / 10;
	vec3 normal = normalize(vec3(d, 1));
	
	vec2 screenCoords = vec2(gl_FragCoord) / vec2(u_screenSize);
	vec3 reflection = texture(u_reflectionTexture, screenCoords + d / 50).rgb;
	...
	float fac = wave_factor(height, u_time) - 0.05*length(d);
	vec4 water = mix(base, waves, fac);
